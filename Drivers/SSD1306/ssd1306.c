...
// Write a number to screenbuffer
void ssd1306_WriteNumber(uint32_t number, uint8_t len, uint8_t fill, FontDef Font, SSD1306_COLOR color) {
  uint8_t start_flag = 0;
  for (uint8_t i = 0; i < len; i++) {
    uint8_t num = (number / Pow(10, len - i - 1)) % 10;
    if (num == 0 && fill == 0 && start_flag == 0) continue;
    ssd1306_WriteChar(num + '0', Font, color);
    start_flag =1;
  }
}
